...
##BS4本身是一种对描述语言进行封装的函数操作模块，通过提供面向对象的操作方式将文档对象中的各种节点、标签、属性、内容等等都封装成了python中对象的属性，在查询操作过程中，通过调用指定的函数直接进行数据匹配检索操作，非常的简单非常的灵活。
...
##调用get_url_content方法，请求"http://10.135.0.8/forum.php?mod=viewthread&tid=5656"，以文本形式返回网页源码
...
##判断ret是否不等于False<html>
<body>
<!--StartFragment--><pre style="margin-top: 0px; margin-right: 0px; margin-bottom: 0px !important; margin-left: 0px; padding: 16px; box-sizing: border-box; font: 400 14px/1.6 Consolas, &quot;Liberation Mono&quot;, Menlo, Courier, monospace; overflow-wrap: normal; overflow: auto; background-color: rgb(247, 247, 247); border-radius: 3px; color: rgb(36, 41, 46); letter-spacing: normal; orphans: 2; text-align: start; text-indent: 0px; text-transform: none; widows: 2; word-spacing: 0px; -webkit-text-stroke-width: 0px; text-decoration-style: initial; text-decoration-color: initial;"><code style="margin: 0px; padding: 0px; box-sizing: border-box; font-family: Consolas, &quot;Liberation Mono&quot;, Menlo, Courier, monospace; font-size: 16px; background: transparent; border-radius: 3px; word-break: normal; white-space: pre; border: 0px; display: inline; overflow: initial; line-height: inherit; overflow-wrap: normal; max-width: none;">import requests
##引入request库， requests是python实现的简单易用的HTTP库，使用起来比urllib简洁很多
from bs4 import BeautifulSoup 
##BS4本身是一种对描述语言进行封装的函数操作模块，通过提供面向对象的操作方式将文档对象中的各种节点、标签、属性、内容等等都封装成了python中对象的属性，在查询操作过程中，通过调用指定的函数直接进行数据匹配检索操作，非常的简单非常的灵活。
from urllib.parse import urlparse
...
#调用get_url_content方法请求目标网站
        if ret != False:
                ...
                print("tid:%s not found,continue" %(i))
##判断ret是否不等于False</code></pre><!--EndFragment-->
</body>
</html>
此次爬虫